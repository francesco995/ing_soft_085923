...
public class PlaceFamilyMemberInMarket2 implements Action {
    ...
    public static void doAction(Board board, Player player, FamilyMember familyMember) throws InvalidObjectException {
        ...
        //Do action
        board.getMarket().get(1).setFamilyMember(familyMember);
        player.add(board.getMarket().get(1).getBoardBonus().getPointsBonus());
        player.add(board.getMarket().get(1).getBoardBonus().getResourcesBonus());
