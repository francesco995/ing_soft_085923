...
public class PlaceFamilyMemberInMarket4 implements Action {
    ...
    public static boolean isValid(Board board, Player player, FamilyMember familyMember){
        ...
        if(!board.getMarket().get(3).isAvailable())
            return false;
    ...
    public static void doAction(Board board, Player player, FamilyMember familyMember) throws InvalidObjectException {
        ...
        //Do action
        board.getMarket().get(3).setFamilyMember(familyMember);
        player.add(board.getMarket().get(3).getBoardBonus().getPointsBonus());
        player.add(board.getMarket().get(3).getBoardBonus().getResourcesBonus());
