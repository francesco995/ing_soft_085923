...
public class PlaceFamilyMemberInMarket3 implements Action {
    ...
    public static void doAction(Board board, Player player, FamilyMember familyMember) throws InvalidObjectException {
        ...
        //Do action
        board.getMarket().get(2).setFamilyMember(familyMember);
        player.add(board.getMarket().get(2).getBoardBonus().getPointsBonus());
        player.add(board.getMarket().get(2).getBoardBonus().getResourcesBonus());
